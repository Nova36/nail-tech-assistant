...
Epic B is the browse-only Pinterest slice. The goal is to let the authenticated user open a Pinterest area inside the app, see their boards, click a board, and see that board's pins using one configured static access token. (Source: `.pHive/planning/prd.md`, `.pHive/planning/sign-off.md`)

The user-story framing is straightforward:

- US-B-1 says the app should use the configured Pinterest account without a user OAuth step, and block the route with token-replacement guidance if the token is missing, revoked, or invalid. (Source: `.pHive/planning/prd.md`)
- US-B-2 says the user should see a board grid, select a board, and then see that board's pin grid. (Source: `.pHive/planning/prd.md`)
...
- The Pinterest UI feels like an extension of Epic A's authenticated shell, not a separate mini-app. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- No broader Pinterest write APIs. (Source: `.pHive/planning/sign-off.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)

The tricky part is that "static token" really means "manual env-managed token," not "forever credential." That matters for UX and ops, even though the product surface is browse-only. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)

## 2. What I Found

Epic A already did a lot of the heavy lifting:

- `middleware.ts` already protects `/pinterest`-shaped routes through the existing authenticated shell matcher. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- `lib/firebase/server.ts` already demonstrates the `import 'server-only';` pattern Epic B should copy. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
- `401`, `403`, `404`, `429`, and generic `5xx` are the important error classes for the UX. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- Add `i.pinimg.com` to `next.config.ts` if using `next/image`. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)

The researcher flagged four concrete gaps, and I agree they are real:

1. `components/` does not exist yet, so Epic B has to bootstrap its own presentational directory. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
2. `lib/utils.ts` is missing, so the common shadcn-style `cn()` helper is not available if we want that pattern. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
3. `PINTEREST_APP_ID` exists in `.env.example` but is not represented in `lib/env.ts`, which means env docs and env schema are out of sync. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
4. The structured outline omits `app/(authenticated)/pinterest/[boardId]/page.tsx`, which is a critical miss because US-B-2 explicitly requires board-detail routing. (Source: `.pHive/planning/structured-outline.md`, `.pHive/planning/prd.md`)
...
- The research explicitly calls this redundant and recommends resolving it before implementation. (Source: `.pHive/planning/structured-outline.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
- Pinterest's full error-code enum beyond `code: 2` was not provided. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
- Normalize HTTP failures into typed errors instead of leaking raw `fetch` semantics everywhere. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
That route pair lines up directly with US-B-2 and avoids forcing everything through API routes that the page itself does not need. (Source: `.pHive/planning/prd.md`)

Boundary shape:

- `app/(authenticated)/pinterest/error.tsx`
- `app/(authenticated)/pinterest/not-found.tsx`
- Optional local `app/(authenticated)/pinterest/[boardId]/not-found.tsx` if we want a board-specific message. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
Mitigation: keep token access inside `lib/pinterest/client.ts`, put `import 'server-only';` on line 1, and keep page data-fetching server-side. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
Mitigation: ship `TokenReplacementGuidance` with terse, copy-pasteable steps, and include direct language that the fix happens in Pinterest developer settings plus Vercel environment variables. (Source: `.pHive/planning/prd.md`, `.pHive/planning/sign-off.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
Mitigation: treat "static" as manual-rotation, surface that in docs and error copy, and ask the user to confirm actual Pinterest dashboard behavior. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
Mitigation: include the `remotePatterns` change in the core implementation, not as cleanup. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
Mitigation: normalize 429 into a retryable browse error and keep the UX explicit. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
Mitigation: start with a load-more button and keep the server client bookmark-aware. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
Mitigation: explicit empty-state copy on `/pinterest` and `/pinterest/[boardId]`. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)
...
- `env.PINTEREST_ACCESS_TOKEN` is already validated at module load. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- The sign-off says that prerequisite path is already the chosen operating model. (Source: `.pHive/planning/sign-off.md`)

One existing Epic A file will need a small follow-up edit:

- `next.config.ts` needs `remotePatterns` for `i.pinimg.com`. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)

Dependency-wise, this slice stays light:

- No new npm dependencies are required.
- Native `fetch` is enough in Next 15.
- No Pinterest SDK is needed. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)

Constraint worth repeating:

- Sign-off #17 explicitly rules out OAuth callback, refresh, and client secret usage, so any design that starts drifting toward those is out of scope by definition. (Source: `.pHive/planning/sign-off.md`)
...
3. Add `lib/utils.ts` with a `cn()` helper now, or defer until a story actually needs shadcn primitives. My opinion: defer. The current app style is already plain Tailwind-first. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)

4. `PINTEREST_APP_ID` cleanup path. Should we add it to `lib/env.ts` schema for parity, or remove it from `.env.example` because the app does not use it? My opinion: remove it from `.env.example` and keep the runtime contract authoritative in `lib/env.ts`. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- The browse contract is just boards plus board-detail pins. (Source: `.pHive/epics/epic-b-pinterest-integration/docs/research-brief.md`)
...
- First-contact debugging against real Pinterest response shapes is likely, especially for board-cover fields where the research confidence is not complete. (Source: `.pHive/planning/prd.md`, `.pHive/epics/epic-b-pinterest-integration/docs/research-raw-findings.md`)

So my recommendation is **medium**.

I would still consider it eligible for a `--fast` path if the team wants to skip extra H/V planning overhead. I would not recommend calling it large. Large would only make sense if this were expanding into OAuth, token refresh, server persistence, or multi-user account linkage, and none of that is in scope. (Source: `.pHive/planning/sign-off.md`)
